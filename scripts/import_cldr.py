...
from _compat import pickle, text_type
from plural import PluralRule
from localedata import Alias

import cldr_numbers
import cldr_dates
from cldr_dates import split_interval_pattern
...
CLDR_ROOT = os.path.join(CHECKOUT_ROOT, "cldr")
        ...
        for elem in format.getiterator():
            if elem.tag == 'dateFormatLength':
                ...
                if _should_skip_elem(elem, type, date_formats):
                    continue
                try:
                    date_formats[type] = cldr_dates.parse_pattern(
        ...
        for elem in format.getiterator():
            if elem.tag == 'timeFormatLength':
                ...
                if _should_skip_elem(elem, type, time_formats):
                    continue
                try:
                    time_formats[type] = cldr_dates.parse_pattern(
            ...
            if elem.tag == 'dateTimeFormatLength':
                ...
                for datetime_skeleton in elem.findall('dateFormatItem'):
                    datetime_skeletons[datetime_skeleton.attrib['id']] = (
                        cldr_dates.parse_pattern(text_type(datetime_skeleton.text))
                    )
...
def parse_decimal_formats(data, tree):
    ...
    for elem in tree.findall('.//decimalFormats/decimalFormatLength'):
        ...
        decimal_formats[type] = cldr_numbers.parse_pattern(pattern)
...
def parse_scientific_formats(data, tree):
    ...
    for elem in tree.findall('.//scientificFormats/scientificFormatLength'):
        ...
        scientific_formats[type] = cldr_numbers.parse_pattern(pattern)
...
def parse_percent_formats(data, tree):
    ...
    for elem in tree.findall('.//percentFormats/percentFormatLength'):
        ...
        percent_formats[type] = cldr_numbers.parse_pattern(pattern)
        ...
        for elem in length_elem.findall('currencyFormat'):
            ...
            for child in elem.getiterator():
                if child.tag == 'alias':
                    ...
                    currency_formats[type] = cldr_numbers.parse_pattern(pattern)
